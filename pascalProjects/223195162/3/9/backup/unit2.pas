...
implementation
 ...
 var i:integer;
   ...
   begin
        tmp:=-1;//
        for i:= 1 to n do
           if (arr[i].time>tmp) then begin tmp:=arr[i].time; c:=arr[i]; end;
   end;
 procedure addRecord(var arr:tmas; var n:integer);
 var i:integer;
     c:theRecord;
   ...
   begin
        ...
        c.album:=inputbox('title', 'ВВЕДИТЕ АЛЬБОМ:','');
        c.time:=strtoint(inputbox('title', 'ВВЕДИТЕ ПРОДОЛЖИТЕЛЬНОСТЬ',''));
        arr[n]:=c;
   ...
   begin
        ...
        for i:=1 to n do begin
          if arr[i].name=s then begin
             ...
             c.firm:=inputbox('title', 'ВВЕДИТЕ НОМЕР КОМНАТЫ','');
             c.album:=inputbox('title', 'ВВЕДИТЕ ДАТУ ЗАСЕЛЕНИЯ:','');
             c.time:=strtoint(inputbox('title', 'ВВЕДИТЕ КОЛИЧЕСТВО ДНЕЙ',''));
             arr[i]:=c;
             end;
        end;
   end;
