...
begin
    ...
    writeln('Количество точек на окружность(по номеру)');
    for i:=1 to numOfC-1 do write(arrayResult[i], ' ' );
    writeln;
    j:=arrayResult[1];//наш счетчик, станет минимумом
    posMin:=1;//счетчик позиции
    for i:=1 to numOfC-1 do
        begin
             if (arrayResult[i]>0) and (arrayResult[i]<j) then
                 begin
                      j:=arrayResult[i];
                      posMin:=i;
                 end;
        end;
    writeln('Искомая окружность под номером: ', i);
end.
